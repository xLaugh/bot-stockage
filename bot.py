...
async def update_coffre_message():
    ...
    if channel:
        ...
        for item in items:
            embed.add_field(name=item[0], value=f"Quantité: {item[1]}", inline=False)
...
async def add_item(ctx, *, item_name):
    ...
    conn.commit()
    await ctx.send(f'Item ajouté à la base de données : {item_name}')
...
async def ajouter_item(ctx, item_name, quantity: int):
    ...
    if item_exists:
        ...
        conn.commit()
        await ctx.send(f'Item ajouté au coffre : {item_name} (Quantité: {quantity})')
        await update_coffre_message()
    else:
        await ctx.send(f"L'item {item_name} n'existe pas dans la base de données.")
...
async def supprimer_item(ctx, item_name, quantity: int):
    ...
    conn.commit()
    await ctx.send(f'Item supprimé du coffre : {item_name} (Quantité: {quantity})')
    await update_coffre_message()
